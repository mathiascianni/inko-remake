        ...
        <>
            ...
            <Container>
                ...
                <ul className="max-w-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 place-items-center">
                    {isLoading ? <Loader /> : data.map(item => (item.featured ? <li key={item.id}><Card type={item.type} id={item.id} name={item.name} price={item.price} src={item.image !== "" ? item.image : "img/not-found.jpg"} /></li> : null))}
                </ul>
            </Container>
        </>
    );

...
const Cart = ({ cartProducts }) => {
    ...
    return (
        <div>
            ...
            <div className={`fixed text-white w-full flex flex-col justify-between lg:w-[40vw] bg-zinc-900/75 backdrop-blur-md h-screen top-0 transtion transition-all duration-1000 z-10 p-2 lg:px-8 pt-24 ` + (active ? "end-0" : "end-[-100%] lg:end-[-40vw]")}>

                <div className="flex border-y p-2 font-bold">
                    ...
                    cartProducts.length > 0 ?
                        <>
                            <ul className="h-full overflow-y-scroll mb-4">
                                {cartProducts.map((product, i) => <li key={i}><CartProduct name={product.name} price={product.price} img={product.image} quantity={product.quantity} /></li>)}
                            </ul>
